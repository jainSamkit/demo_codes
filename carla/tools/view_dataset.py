...
if __name__ == "__main__":
  ...
  path = '/home/fcodevil/Datasets/GTA/27_IrisN_Seoul_2_1/'
  ...
  for h_num in positions_to_test:
    ...
    for i in range(200):
      ...
      direction = np.array(data['targets'][i][18])
     ...
     #print predicted_steers
      ...
      print 'Steer -> ',data['targets'][i][0], '    Yaw -> ',data['targets'][i][12]
      output_file.write('%f,%f\n' % (data['targets'][i][12],time.time()))
      ...
      screen.blit(myfont.render(str(data['targets'][i][16]), 1, (0,255,0)), (100, 200))
      posx = data['targets'][i][8]
      posy =data['targets'][i][9]
      goal_pos_x = data['targets'][i][17]
      goal_pos_y = data['targets'][i][18]
      dist = math.sqrt((posx-goal_pos_x)*(posx-goal_pos_x)  + (posy-goal_pos_y)*(posy-goal_pos_y))
