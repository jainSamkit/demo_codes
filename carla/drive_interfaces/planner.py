...
class Planner(object):
	...
	def make_node(self,worldvertex):
		...
		rotation = np.array([worldvertex[0],worldvertex[1],worldvertex[2]])
	...
	def graphToListOfPoints(self, orientation):
		pass


	# Get the next command by calling graph to comands
	...
	def get_next_command(self,source,source_ori,  target,target_ori):
		...
		if distance_node >1 and self.previous_source != node_source:
			...
			self.commands = self.graph_to_commands(route)
