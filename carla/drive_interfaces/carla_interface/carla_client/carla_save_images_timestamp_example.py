...
def pack_summary(pack,start_time):
    return 'TIME_PASSED %.3f | TIME_PASSED this episode %.3f |  image number %d | player %.3f %.3f | goal %.3f %.3f ' % \
            (pack.timestamp,pack.timestamp -start_time, pack.image_number, pack.player_position[0], pack.player_position[1], pack.objective[0], pack.objective[1])


def run_n_steps_timestamp(client, num_steps, restart_freq=None, restart_close_to_goal=None, write_images_to='', verbose=0, gas=1.0, steer=0.0, log_detailed='', log_oneliner=''):
        ...
        for step in range(num_steps):
            #beforepack = time.time()
            pack = client.get_data()
            #print (time.time()-beforepack)
            ...
            if len(timestamps) < 2 or pack.timestamp != timestamps[-2]:
                ntimestep += 1
                if len(timestamps) < 2:
                    previous_tstamp = pack.timestamp
                else:
                    previous_tstamp = timestamps[-2]
                print(('step %d, timestep %d  interval %.3f: ' % (step, ntimestep,(pack.timestamp - previous_tstamp) )) + pack_summary(pack,start_stamp))
                if write_images_to:
                    if ntimestep == 1:
...
if __name__ == '__main__':
    ...
    run_n_steps_timestamp(carla, args.num_steps, restart_freq=args.restart_frequency, restart_close_to_goal=args.restart_close_to_goal, \
