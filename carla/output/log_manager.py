...
class LogManager(object):
	...
	def update_log_state(self,i,duration):
		...
		feedDict = self._training_manager.get_feed_dict()
		#epoch_number = (float(i)*float(self._batch_size))/float(self._config.number_images_epoch)
	...
	def write_variable_error(self):
		...
		for i in self._config.targets_to_print:
			...
			split_target = tf.split(output_tensor,output_tensor.get_shape()[1],1)
			print split_target
			index_target =self._config.branch_config[self._config.selected_branch].index(i)
			targets = self._sess.run(split_target[index_target], feed_dict=feedDict)
