...
def create_structure(tf, input_image, input_size,dropout,config): #removes input_data to match my(akshay) code. unused in this code anyway
	...
	for i in range(0,len(config.branch_config)):  
		with tf.name_scope("Branch_" + str(i)):
			if config.branch_config[i][0] == "Speed":
				...
				branch_output =network_manager.fc_block(branch_output,16)
			else:
